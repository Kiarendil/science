...
f = ROOT.TFile('./B_cut.root', 'read');  # file where to read from

histname = "hBmas"

mbmin = 5.06; mbmax = 5.46; mbNbins = 150 # range of plot + N(bins)

varname = "M(B) , [GeV]"
TwoGaus = 1

canvW = 1000; canvH = 600
_PicName = 'B_mass_Fit_4.png'  ## name of file with picture

fit_M0 = 5.27963; fit_Sigma0 = 0.015; fit_N0 = 1364 ## start positions for fit
...
alist1  = ROOT.RooArgList (pdfS1, pdfB);  alist2 = ROOT.RooArgList (S1, B);

if TwoGaus  :
   alist1  = ROOT.RooArgList (pdfS1, pdfS2, pdfB);  alist2 = ROOT.RooArgList (S1, S2, B);
...
pdfSum.plotOn(mframe, ROOT.RooFit.Components('pdfB'), ROOT.RooFit.LineColor(ROOT.kBlue), ROOT.RooFit.LineStyle(ROOT.kDashed), ROOT.RooFit.LineWidth(2))
